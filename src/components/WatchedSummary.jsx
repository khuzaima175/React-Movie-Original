...
import { Film, Star, Award, Clock, Trophy } from "lucide-react";
...
export default function WatchedSummary({ watched }) {
  ...
  const stats = [
    { icon: <Film size={18} aria-hidden="true" />, value: watched.length, label: "movies" },
    { icon: <Star size={18} className="icon-gold" aria-hidden="true" />, value: watched.length > 0 ? avgImdbRating : "0.0", label: "IMDb" },
    { icon: <Award size={18} className="icon-award" aria-hidden="true" />, value: watched.length > 0 ? avgUserRating : "0.0", label: "Your avg" },
    { icon: <Clock size={18} aria-hidden="true" />, value: `${hours}h ${mins}m`, label: "watched" },
  ];
  ...
  return (
    <motion.div
      ...
      aria-label="Watched movie statistics"
    >
      ...
      {topRatedMovies.length > 0 && maxRating > 0 && (
        ...
        >
          <Trophy size={16} className="trophy-icon" aria-hidden="true" />
